...
import { fetchVerifyWalletEmail } from "./service"
 ...
 */
export function useVerifyWalletEmail() {
  /**
  ...
  const { isPending, mutate, mutateAsync } = useMutation<
    ...
    VerifyWalletEmailInputDto
  >({
    mutationFn: fetchVerifyWalletEmail,
  })

  return {
    verifyWalletEmail: mutate,
    isPending,
    verifyWalletEmailAsync: mutateAsync,
  }
}
